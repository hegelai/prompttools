...
class GoogleVertexChatCompletionExperiment(Experiment):
    ...
    def palm_completion_fn(self, **input_args):
        chat_model = ChatModel.from_pretrained(input_args["model"])
        chat = chat_model.start_chat(input_args["context"], input_args["examples"])
        params = copy.copy(input_args)
        del params["model"], params["context"], params["examples"]
        return chat.send_message(input_args["messages"], **input_args)

    @staticmethod
    def _extract_responses(completion_response) -> list[str]:
