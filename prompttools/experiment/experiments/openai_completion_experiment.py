...
class OpenAICompletionExperiment(Experiment):
    ...
    Args:
        ...
        logprobs (list[int]):
            ...
            chosen tokens. For example, if ``logprobs`` is 5, the API will return a list of the 5 most likely tokens.
        ...
        logit_bias (list[dict]):
            ...
            from -100 to 100. Mathematically, the bias is added to the logits generated by the model prior to
            sampling. The exact effect will vary per model, but values between -1 and 1 should decrease or increase
            the likelihood of selection; values like -100 or 100 should result in a ban or exclusive selection of
            the relevant token.
    """
