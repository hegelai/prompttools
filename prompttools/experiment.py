...
def extract_chat_responses(output) -> str:
...
class OpenAIChatExperiment:
    ...
    def run(self):
        ...
        for combo in self.argument_combos:
            self.result.append(openai.ChatCompletion.create(model=combo[0],
                                                            messages=combo[1],
                                                            temperature=combo[2],
                                                            top_p=combo[3],
                                                            n=combo[4],
    ...
    def evaluate(self, eval_fn: Callable):
        ...
        for i, result in enumerate(self.results):
            self.scores.append(eval_fn(self.argument_combos, result))
    ...
    def visualize(self):
        ...
        table = pd.DataFrame({'model': [combo[0] for combo in self.argument_combos],
                              'messages': [combo[1] for combo in self.argument_combos],
                              'response(s)': extract_chat_responses(self.results),
                              'score': self.scores,
